...
RF rf;
static uint32_t rx_timeout_tick = 0;
static uint32_t tx_timeout_tick = 0;
tSX1276* SX1276;
tSX1276LR* SX1276LR;
...
{
return HAL_GPIO_ReadPin(DIO2_GPIO_Port, DIO2_Pin);
}
...
{
return HAL_GPIO_ReadPin(DIO4_GPIO_Port, DIO4_Pin);
}

inline uint8_t SX1276ReadDio5( void )
{
return HAL_GPIO_ReadPin(DIO5_GPIO_Port, DIO5_Pin);
}
...
//	SysTick->VAL =0x00;    
//  SysTick->CTRL|=SysTick_CTRL_ENABLE_Msk ;
	rx_timeout_tick = HAL_GetTick() + LoRaSettings.RxPacketTimeout;
//	 rx_timeout_tick = HAL_GetTick() + LoRaSettings.RxPacketTimeout;
}
...
//	SysTick->VAL =0x00;        
//  SysTick->CTRL|=SysTick_CTRL_ENABLE_Msk ;
	 tx_timeout_tick = HAL_GetTick() + LoRaSettings.TxPacketTimeout;
//	tx_timeout_tick = HAL_GetTick() + LoRaSettings.TxPacketTimeout;
}
...
//													}
											if(HAL_GetTick() >= rx_timeout_tick) 
        {
            RFLRState = RFLR_STATE_RX_TIMEOUT;
        }
										}
...
//													}
											 if(HAL_GetTick() >= tx_timeout_tick) 
        {
            RFLRState = RFLR_STATE_TX_TIMEOUT;
        }
										}
